...
var _RunOverviewStore = __webpack_require__(2111);

var _RunOverviewStore2 = _interopRequireDefault(_RunOverviewStore);
...
var RunOverview = (_dec = (0, _mobxReact.inject)('kanbanStore'), _dec(_class = (0, _mobxReact.observer)(_class = function (_React$Component) {
	_inherits(RunOverview, _React$Component);

	function RunOverview(props) {
		_classCallCheck(this, RunOverview);

		var _this = _possibleConstructorReturn(this, (RunOverview.__proto__ || Object.getPrototypeOf(RunOverview)).call(this, props));
		...
		_this.fillTableColumn = function (item) {
			if (item.name === "整体经营总览") {
				if (item.schemas) {
					...
					var current = item.schemas.find(function (item) {
						return item.name === "current";
					});
					_this.store.tableColumnCurrent = current.title;
				}
			}
		};
		...
		_this.store = new _RunOverviewStore2.default();
		_this.state = {
			id: '', // 表id
			value: '年度',
			value1: '2019',
   ...
   * */
		...
		value: function render() {
			...
			var columns = [{
				...
				render: function render(text, row, index) {
					if (index === 0 || index === 3 || index === 8) {
						return {
							...
							props: {
								rowSpan: 3,
								className: 'tab_td_bg td_color vt w150'
							}
						};
						...
						return {
							...
							props: {
								colSpan: 2,
								className: 'tab_td_bg td_color vt'
							}
						};
					} else {
						return {
				...
				// 	if (typeof(text)==="object" && text.hasOwnProperty("rows")) {
				// 		return {
				// 			children: <a>{text.label}</a>,
				// 			props: {
				// 				rowSpan: text.rows,
				// 				className:'tab_td_bg'
				// 			}
				// 		};
				// 	}
				// 	if (typeof(text)==="object" && text.hasOwnProperty("cols")) {
				// 		return {
				// 			children: <a>{text.label}</a>,
				// 			props: {
				// 				colSpan: text.cols,
					...
					if (index === 6 || index === 7) {
						...
						return {
							children: _react2.default.createElement(
								'a',
								{ href: '#/kanban/rlcb?type=home&key=1&id=13&time=' + time + '&month=' + month },
								text
							),
							props: {
								className: 'td_bg1 vt td_color'
						...
						return {
							children: _react2.default.createElement(
								'a',
								{ href: '#/kanban/zxfy?type=home&key=1&id=15&time=' + time + '&month=' + month },
								text
							),
							props: {
								className: 'td_bg1 vt td_color'
						...
						return {
							children: _react2.default.createElement(
								'a',
								{ href: '#/kanban/jcfy?type=home&key=1&id=16&time=' + time + '&month=' + month },
								text
							),
							props: {
								className: 'td_bg1 vt td_color'
				...
				// render: (text, row, index) => {
				// 	if (row.region.label=="税费" || row.region.label=="净利润") {
				// 		return {
				// 			props: {
				// 				colSpan: 0,
				// 				className:'tab_td_bg'
				...
				title: this.store.tableColumnCurrent,
				dataIndex: 'current',
				align: 'center',
				className: 'vt'
			}, {
...
var RunOverviewStore = (_class = function () {
	function RunOverviewStore() {
		_classCallCheck(this, RunOverviewStore);

		_initDefineProp(this, 'overviewTableData', _descriptor, this);

		_initDefineProp(this, 'overviewTableColumn', _descriptor2, this);

		_initDefineProp(this, 'tableColumnCurrent', _descriptor3, this);

		_initDefineProp(this, 'lastUpdatedTime', _descriptor4, this);
	} // 整体情况表体数据

	//整体表头数据
	//表头 当期

	_createClass(RunOverviewStore, [{
	...
	return RunOverviewStore;
}(), (_descriptor = _applyDecoratedDescriptor(_class.prototype, 'overviewTableData', [_mobx.observable], {
	enumerable: true,
	initializer: function initializer() {
		return [];
	}
}), _descriptor2 = _applyDecoratedDescriptor(_class.prototype, 'overviewTableColumn', [_mobx.observable], {
	enumerable: true,
	initializer: function initializer() {
		return [];
	}
}), _descriptor3 = _applyDecoratedDescriptor(_class.prototype, 'tableColumnCurrent', [_mobx.observable], {
	enumerable: true,
	initializer: function initializer() {
		return [];
	}
}), _descriptor4 = _applyDecoratedDescriptor(_class.prototype, 'lastUpdatedTime', [_mobx.observable], {
	enumerable: true,
	initializer: function initializer() {
		return "";
	}
...
exports.default = RunOverviewStore;

/***/ })

});
//# sourceMappingURL=33-afa7d4b4bae6ff2924e9.1629092961041.js.map