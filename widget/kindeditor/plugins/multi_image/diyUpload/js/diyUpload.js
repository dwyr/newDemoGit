		...
		*/
		diyUpload: function (opt, serverCallBack) {
			...
			webUploader.on('uploadBeforeSend', function (object, data, headers) {
                ...
                const {file} = object;
				const {source} = file;
				const newkey = 'ZJMZ_OpenPlatform/' + getDate() + "/" + source.ruid+"/"+file.name;
				...
				$.ajax({
					...
					success : function (res) {
						...
						data.OSSAccessKeyId = token.accessid;
			...
			//上传成功后触发事件;
			webUploader.on('uploadSuccess', function (files, response) {
				var $fileBox = $('#fileBox_' + files.id);
				var $diyBar = $fileBox.find('.diyBar');
				...
				const {source} = files;
				const newkey = '/ZJMZ_OpenPlatform/' + getDate() + "/" + source.ruid+"/"+files.name;
				// 替换成功后的url链接
				$fileBox.find("img").attr("src", opt.server + newkey);
			});
