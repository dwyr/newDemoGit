...
var officeArray = ["weekreport"]; //  公文

var Notify = (_dec = (0, _mobxReact.inject)('DigtalOrgStore'), _dec2 = (0, _mobxReact.inject)('OfficeManageStore'), _dec(_class = _dec2(_class = (0, _mobxReact.observer)(_class = function (_Component) {
	...
	function Notify(props) {
				...
				return regeneratorRuntime.wrap(function _callee$(_context) {
					while (1) {
						switch (_context.prev = _context.next) {
							case 0:
								_this.store.notifyParams.pageIndex = 1;
								_this.store.notifyParams.pageSize = pageSize;
								_this.store.getNotifyPageList();
				...
				return regeneratorRuntime.wrap(function _callee2$(_context2) {
					while (1) {
						switch (_context2.prev = _context2.next) {
							case 0:
								_this.store.notifyParams.pageIndex = pageNumber;
								_this.store.getNotifyPageList();
		...
		_this.click = function (type) {
			_this.setState({ selectedRowKeys: Object.assign([]) });
			_this.store.notifyParams.pageIndex = 1;
			_this.store.notifyParams.pageSize = 10;
			_this.store.notifyParams.type = type;
			_this.store.getNotifyPageList();
		};

		_this.delNotifyList = function () {
			var selectedRowKeys = _this.state.selectedRowKeys;

			if (selectedRowKeys.length == 0) {
				_antd.message.warn("请先选择要删除的数据");
				return false;
			}
			_this.store.delNotifyList(selectedRowKeys).then(function (res) {
				if (res) {
					...
					_this.setState({
						...
						// 重查
						_this.store.getNotifyPageList();
						_events2.default.emit("notify", "6");
					});
				}
			});
		};

		_this.setNotifyListRead = function () {
			var selectedRowKeys = _this.state.selectedRowKeys;

			if (selectedRowKeys.length == 0) {
				_antd.message.warn("请先选择要标记的数据");
				return false;
			}
			_this.store.setNotifyListRead(selectedRowKeys).then(function (res) {
				if (res) {
		...
		_this.onOpen = function (record) {
			if (record.mainType == null || record.mainType == "") {
				// 已删除
				_antd.message.warn("日志已被删除");
				if (record.haveRead != 1) {
					// 未读
					_this.store.setNotifyRead(record.id).then(function (res) {
						if (res) {
							// 重查
							_this.store.getNotifyPageList();
							_events2.default.emit("notify", "6");
			...
			if (officeArray.includes(record.mainType)) {
				//公文管理
				_this.officeStore.getJournalTypeList().then(function (res) {
					_this.officeStore.temList = Object.assign({}, mobx.toJS(_this.officeStore.digtalTemData).filter(function (item) {
						return item.type == record.mainType;
					})[0]);
				});
				_this.officeStore.selestedId = record.mainId;
				_this.officeStore.getJournalById(record.mainId).then(function (res) {
					if (res) {
						_this.setState({
							visibleOffic: true,
							...
							_this.officeStore.detailData = Object.assign({}, res);
						});
					}
				});
				// 点赞
				_this.officeStore.getLikeUsersByJournalId();
				// 评论
				_this.officeStore.getCommentsByJournalId();
				_this.officeStore.getJournalIndicators();
				_this.officeStore.isEdit = false;
			} else {
				//知识管理
				_this.store.getJournalTypeList().then(function (res) {
					_this.store.temList = Object.assign({}, mobx.toJS(_this.store.digtalTemData).filter(function (item) {
						return item.type == record.mainType;
					})[0]);
				});
				_this.store.selestedId = record.mainId;
				_this.store.getJournalById(record.mainId).then(function (res) {
					if (res) {
						_this.setState({
							visible: true,
							selectedID: record.id,
				...
				// 评论
				_this.store.getCommentsByJournalId();
				_this.store.getJournalIndicators();
				_this.store.isEdit = false;
			}
		};
		...
		_this.onClose = function (type) {
			...
			_this.setState(_defineProperty({}, type, false), function () {
				_this.store.setNotifyRead(selectedID).then(function (res) {
					if (res) {
						// 重查
						_this.store.getNotifyPageList();
						_events2.default.emit("notify", "6");
		...
		_this.officeStore = _this.props.OfficeManageStore;
		_this.state = {
			selectedRowKeys: [],
			selectedID: "",
			visible: false,
	...
	_createClass(Notify, [{
		...
		value: function render() {
			...
			var _store = this.store,
			    notifyTotalCount = _store.notifyTotalCount,
			    notifyTableData = _store.notifyTableData,
			    notifyParams = _store.notifyParams,
			    notifyLoading = _store.notifyLoading;

			var colorLists = ['#f56a00', '#7265e6', '#ffbf00', '#00a2ae', '#F08080', '#EEE685', '#9400D3', '#D1EEEE', '#90EE90', "#9400D3"]; // 十个颜色
			var columns = [{
				title: '',
				dataIndex: 'name',
				key: 'name',
				render: function render(text, record) {
					var haveRead = record.haveRead,
								...
								return _react2.default.createElement(
									_antd.List.Item,
									{
										...
										extra: _react2.default.createElement(
											...
											(0, _utils.showText)((0, _moment2.default)(createTime).format("YYYY-MM-DD HH:mm"), (0, _moment2.default)(createTime).format("HH:mm"))
										)
									},
									_react2.default.createElement(_antd.List.Item.Meta, {
										...
										title: _react2.default.createElement(
											...
											haveRead == 0 ? _react2.default.createElement(_antd.Badge, { status: "warning", text: fromUserName }) : fromUserName,
											'\u5728',
											_react2.default.createElement(
												'span',
												{ style: { color: "#1890ff", cursor: 'pointer' }, onClick: _this3.onOpen.bind(_this3, record) },
												mainTitle
											),
											'\u4E2D',
											canShow == 1 && msgState == 2 ? _react2.default.createElement(
												_antd.Tag,
												{ color: 'gold' },
												_this3.transType(msgType),
												'\u5DF2\u88AB\u4F5C\u8005\u5220\u9664'
											) : null
										),
										description: _react2.default.createElement(
											...
											_react2.default.createElement(
												...
												{ style: {
														...
														fontSize: '14px',
														color: haveRead == "1" ? "#D5D3D6" : "#79797B" } },
												canShow == 0 ? _react2.default.createElement(
													_antd.Tag,
													{ color: 'gold' },
													_this3.transType(msgType),
													'\u5DF2\u88AB\u4F5C\u8005\u5220\u9664'
			...
			return _react2.default.createElement(
				...
				_react2.default.createElement(
					_antd.Button,
					{ type: type == 2 ? "primary" : "default", shape: 'round',
						onClick: this.click.bind(this, 2) },
					'\u5168\u90E8'
				),
				_react2.default.createElement(
					_antd.Button,
					{ type: type == 0 ? "primary" : "default", shape: 'round', className: 'ml10 mr10',
						onClick: this.click.bind(this, 0) },
					'\u672A\u8BFB'
				),
				_react2.default.createElement(
					_antd.Button,
					{ type: type == 1 ? "primary" : "default", shape: 'round',
						onClick: this.click.bind(this, 1) },
					'\u5DF2\u8BFB'
				),
				_react2.default.createElement(_antd.Divider, { style: { marginBottom: "0" } }),
				_react2.default.createElement(
					'div',
					{ className: 'fr', style: {
							position: "relative",
					...
					_react2.default.createElement(
						_antd.Button,
						{ type: 'primary', ghost: true, icon: 'delete', size: "small", onClick: this.delNotifyList },
						'\u6279\u91CF\u5220\u9664'
					),
					_react2.default.createElement(
						_antd.Button,
						{ className: 'ml20', type: 'primary', ghost: true, icon: 'eye', size: "small",
...
//# sourceMappingURL=9-afa7d4b4bae6ff2924e9.1629092961041.js.map